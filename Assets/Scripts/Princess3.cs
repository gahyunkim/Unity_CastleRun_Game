...
using UnityEngine.SceneManagement;   //씬 이동을 하기 위한 부분
...
public class Princess3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (transform.position.y < -7)  //만약 공주의 y값이 -7보다 작아진다면 게임 오버하는 씬으로 이동
        {
            SceneManager.LoadScene("3GameOver");   //3GameOver씬으로 이동하는 부분
        }
